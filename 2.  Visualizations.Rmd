...
countries %>%
  ...
  labs(title = "Contabilidad del crecimiento",
       subtitle = "Contribución de los factores productivos por país (1985 – 2019)",
       caption = "Datos: Penn World Table (v 10.0)") +
  guides(color = guide_legend(nrow = 1)) +
  scale_fill_discrete(name = "Contribución:",
                      labels = c("Capital", "Trabajo", "PTF")) +
...
```{r}
ggsave(filename = here("Plots","Plot4.png"))
```

## (g) Worldmaps

Se necesita agregar a los países que faltan por llamarse de forma distinta en ambas bases de datos, además el mapa es interesante si y solo si denota en qué países contribuye en su mayoría al crecimiento el factor capital, el factor trabajo y la productividad total de los factores.

Aquí está como arreglar los nombres de los países: <https://www.sharpsightlabs.com/blog/map-oil-production-country-r/>
...
mapdata %>% 
  ...
  geom_polygon(aes(fill = k_cont), color = "black", size = .1) +
  coord_cartesian(ylim = c(-50, 80)) +
  guides(color = guide_legend(nrow = 1)) +
  labs(title = "Contribución de la productividad del capital",
       subtitle = "en el crecimiento económico de 1985 a 2019",
       caption = "Datos: Penn World Table (v 10.0)") +
  scale_fill_continuous(name = "Capital") +
  scale_y_continuous() +
  guides(color = guide_legend(nrow = 1)) +
  theme(
    axis.title.x = element_blank(),
